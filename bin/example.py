...
if __name__ == "__main__":
    ...
    for i in range(0):
        mesh = hrbf_surface_py.reconstruct_surface(pcl, args.partition_size, args.partition_overlap, args.voxel_size, single_partition, i)
        # Plot points
        ax.scatter(mesh.pv[:,0], mesh.pv[:,1], mesh.pv[:,2], color='b', s=20)
        edges = [
            (0, 1), (1, 5), (5, 4), (4, 0), # Bottom square
            (2, 3), (3, 7), (7, 6), (6, 2), # Top square
            (0, 2), (1, 3), (5, 7), (4, 6)  # Vertical pillars
        ]
    ...
    mesh = hrbf_surface_py.reconstruct_surface(pcl, args.partition_size, args.partition_overlap, args.voxel_size, single_partition, -1)
    if len(mesh.positions):
        ax.plot_trisurf(mesh.positions[:,0], mesh.positions[:,1], mesh.positions[:,2], triangles=mesh.faces, cmap='Spectral')

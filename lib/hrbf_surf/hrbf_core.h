 ...
 */
/// @file hrbf_core.hpp
/// Original author: Gael Guennebaud - gael.guennebaud@inria.fr -
/// http://www.labri.fr/perso/guenneba/ Rodolphe Vaillant - (Fixed the gradient
/// evaluation) - http://www.irit.fr/~Rodolphe.Vaillant
...
/// must implement float Rbf::f(float) float Rbf::df(float) float
/// Rbf::ddf(float) (see hrbf_phi_funcs.h for an example)
/// @note Please go see http://eigen.tuxfamily.org to use matrix and vectors
/// types of this lib. The documentation is pretty good.
template <typename _Scalar, int _Dim, typename Rbf> class HRBF_fit {
public:
  ...
  void hermite_fit(const std::vector<Vector> &points,
    ...
    for (int i = 0; i < nb_points; ++i) {
      ...
      for (int j = 0; j < nb_points; ++j) {
        ...
        if (l == 0) {
          ...
          D.template block<Dim, Dim>(io + 1, jo + 1) =
    ...
    D.diagonal().array() += 1e-8;

    x = D.ldlt().solve(f);
    Eigen::Map<Eigen::Matrix<Scalar, Dim + 1, Eigen::Dynamic>> mx(
        x.data(), Dim + 1, nb_points);
  ...
  Vector grad(const Vector &x) const {
    ...
    for (int i = 0; i < nb_nodes; i++) {
      ...
      if (l > 0.00001f) {
        ...
        grad += bDotd_l * (ddphi * diffNormalized - diff * dphi / squared_l) +
                beta * dphi / l;
      }
    }
    return grad;
